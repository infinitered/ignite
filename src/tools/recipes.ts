...
export async function applyRecipe(recipe: CookbookRecipe, api_key: string): Promise<void> {
  // start with asking OpenAI for the first step
  ...
  const messages: ChatCompletionMessageParam[] = [
    {
      ...
      content: `
        ...
        The user will now give you the recipe. Please execute the first step
        or communicate why you cannot to the user.
      ...
      for (const tool_call of tool_calls) {
        ...
        switch (tool_call.function.name) {
          ...
          case "patchFile":
            ...
            print.info(args)
            print.error("Patching files is not yet supported")
            process.exit(1)
            break
          case "listFiles":
